...
func MakeTemporaryStatePrepper(cfg *utils.Config) executor.Extension[txcontext.TxContext] {
	var cache cc.Cache[statedb.CodeKey, common.Hash]
	var mutex = new(sync.Mutex)
	if cfg.Cache > 0 {
		cache = cc.NewLruCache[statedb.CodeKey, common.Hash](cfg.Cache)
	}

	switch cfg.DbImpl {
	case "in-memory", "memory":
		return &temporaryInMemoryStatePrepper{cache: cache, mutex: mutex}
	case "off-the-chain":
		fallthrough
	default:
		// offTheChainStateDb is default value
		substate.RecordReplay = true
		return &temporaryOffTheChainStatePrepper{cfg: cfg, cache: cache, mutex: mutex}
	}
}
...
type temporaryInMemoryStatePrepper struct {
	...
	cache cc.Cache[statedb.CodeKey, common.Hash]
	mutex *sync.Mutex
}
...
func (p *temporaryInMemoryStatePrepper) PreTransaction(state executor.State[txcontext.TxContext], ctx *executor.Context) error {
	...
	ctx.State = statedb.MakeInMemoryStateDB(alloc, uint64(state.Block), p.cache, p.mutex)
	return nil
}
...
func (p *temporaryOffTheChainStatePrepper) PreTransaction(state executor.State[txcontext.TxContext], ctx *executor.Context) error {
	...
	ctx.State, err = statedb.MakeOffTheChainStateDB(state.Data.GetInputState(), uint64(state.Block), statedb.NewChainConduit(p.cfg.ChainID == utils.EthereumChainID, utils.GetChainConfig(utils.EthereumChainID)), p.cache, p.mutex)
	return err
}
